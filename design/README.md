...
- `av-steve/creeper/axolotl/panda/fox.png` — 32×32 avatar faces
- `av-bear.png`, `av-shadow.png`, `av-stormy.png` — 32×32 avatars of Haley's three pets (see "Pet avatars" below). **These replace `av-cat`, `av-alex` and `av-bee` in the picker.** The three retired files are still in `assets/` but are no longer referenced by any screen.
...
The parent zone is iron and neutral. The rule underneath that: **green means Haley acted; gold means coins are moving.** Neither is about who is holding the phone.

- **Green is never a parent-initiated action** — no exceptions. Nothing a parent taps in order to *do* something is green. Green only ever reports what Haley did (e.g. the review list's marks in 1k, which are a record of her taps and stay green).
- **Gold is permitted anywhere coins actually move**, parent zone included: "Approve & award N", "Confirm & award night coins", "Redeem coins". A gold button that *cannot* pay is the exception — the short-of-threshold "Redeem · 1,720 to go" takes recessed iron.
- Everything else in the parent zone — toggles, Save, ＋ Add, editable values, focus rings — is iron, stone, or slate.
...
**Checkbox states — three facts, three materials.**

| State | Material | Where |
|---|---|---|
| Haley did it | Green bevelled slot, white ✔ | Her checklist; the 1k review list's marks |
| A grown-up recorded it | Flat iron `#6f6f6f`, 3px `#4a4a44`, white ✔, no bevel highlight | The parent-verified row's live toggle in 1k |
| Locked, not yet decided | Flat iron, 14–16px `#4a4a44` inset square, no bevel highlight, not tappable | Haley's view of a parent-verified row |

A parent scanning the review list should be able to tell at a glance which lines are Haley's word and which are their own, so the row they set themselves must not use her green.
...
Three new avatars drawn from photographs of Haley's own pets, replacing `av-cat`, `av-alex` and `av-bee`. Same 32×32 canvas as the rest of the set, same flat palette, no anti-aliasing, `image-rendering: pixelated`.

| File | Pet | Reading | Replaces |
|---|---|---|---|
| `av-bear.png` | Bear, bernedoodle | Mob head — tricolour face, white blaze forehead to muzzle, tan brows, amber eyes | `av-cat.png` |
| `av-shadow.png` | Shadow, cat | Sitting full body — silver ruff, cream chest, blue slit-pupil eyes, tail curled to the front paws | `av-alex.png` |
| `av-stormy.png` | Stormy, rat | Mob head — tapering face, big round ears set low on the sides, black eyes with a glint | `av-bee.png` |

Shadow is a full body while the other two are heads. That is deliberate — her silhouette is the most recognisable thing about her, and a head-only version lost the ruff. The picker frames are square and the art is centred inside, so mixing head and full-body avatars in one grid is fine.

**Grid position is not meaningful.** The avatar picker in 1n is an 8-slot grid; the three new files simply occupy the slots the retired three used to. Order can change freely.

**Still referencing a retired file:** the "Nothing to do right now" empty state on Home (1c) uses `av-bee.png` as decoration. It is not an avatar there, so it was left alone — say the word if it should change too.

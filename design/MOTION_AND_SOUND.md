...
| 200ms | Next progress pip flips green | `transform: scaleX(0 → 1)`, `transform-origin: left`, 160ms. On the routine card's pip row. |
...
| **Timer start** | The bar snaps to full over 200ms, then begins draining, so starting reads as "loaded". Presets cross-fade out. |
...
- **Auto-quiet during the Nighttime Routine window and after it.** The phone is in her room. The alarm chime is the one exception and stays at its current gain.
